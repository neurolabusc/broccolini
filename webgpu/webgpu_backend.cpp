/* webgpu_backend.cpp — Adapts webgpu_registration to the C backend vtable */

#include "webgpu_backend.h"
#include "webgpu_registration.h"
#include <cstring>

/* ========== Convert broc_filters → webgpu_reg::QuadratureFilters ========== */

static webgpu_reg::QuadratureFilters convert_filters(const broc_filters *f)
{
    webgpu_reg::QuadratureFilters qf;
...
static broc_result webgpu_register_volumes(
    broc_backend       *self,
    ...
    const broc_reg_params *params)
{
    ...
    webgpu_reg::QuadratureFilters qf = convert_filters(filters);
    webgpu_reg::VolumeDims idims = { in_dims.W, in_dims.H, in_dims.D };
    webgpu_reg::VoxelSize  ivox  = { in_vox.x, in_vox.y, in_vox.z };
    webgpu_reg::VolumeDims rdims = { ref_dims.W, ref_dims.H, ref_dims.D };
    webgpu_reg::VoxelSize  rvox  = { ref_vox.x, ref_vox.y, ref_vox.z };
    ...
    webgpu_reg::T1MNIResult mr = webgpu_reg::registerT1MNI(
        input, idims, ivox,
        ref, rdims, rvox,
...
static void webgpu_destroy(broc_backend *self)
{
    free(self);
}
...
extern "C" broc_backend *broc_webgpu_create_backend(void)
{
    broc_backend *b = (broc_backend *)calloc(1, sizeof(broc_backend));
    if (!b) return NULL;

    b->register_volumes = webgpu_register_volumes;
    b->destroy          = webgpu_destroy;
    b->name             = "WebGPU";
    b->priv             = NULL;
