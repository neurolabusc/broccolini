/* opencl_backend.cpp — Adapts opencl_registration to the broccolini C vtable.
 *
 * Follows the same adapter pattern as Metal, CUDA, and WebGPU backends.
 * Requires BROCCOLI_DIR env var pointing to opencl/ so OpenCL kernel
 * source files can be found at $BROCCOLI_DIR/kernels/*.cpp.
 */
...
/* ========== Convert broc_filters → opencl_reg::QuadratureFilters ========== */

static opencl_reg::QuadratureFilters convert_filters(const broc_filters *f)
{
    opencl_reg::QuadratureFilters qf;
    ...
    const broc_reg_params *params)
{
    ...
    opencl_reg::QuadratureFilters qf = convert_filters(filters);
    opencl_reg::VolumeDims idims = { in_dims.W, in_dims.H, in_dims.D };
    opencl_reg::VoxelSize  ivox  = { in_vox.x, in_vox.y, in_vox.z };
    opencl_reg::VolumeDims rdims = { ref_dims.W, ref_dims.H, ref_dims.D };
    opencl_reg::VoxelSize  rvox  = { ref_vox.x, ref_vox.y, ref_vox.z };
    ...
    opencl_reg::T1MNIResult mr = opencl_reg::registerT1MNI(
        input, idims, ivox,
        ref, rdims, rvox,
