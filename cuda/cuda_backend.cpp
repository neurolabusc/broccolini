/* cuda_backend.cpp — Adapts cuda_registration.cu to the C backend vtable */

#include "cuda_backend.h"
#include "cuda_registration.h"
#include <cstring>

/* ========== Convert broc_filters → cuda_reg::QuadratureFilters ========== */

static cuda_reg::QuadratureFilters convert_filters(const broc_filters *f)
{
    cuda_reg::QuadratureFilters qf;
...
static broc_result cuda_register_volumes(
    broc_backend       *self,
    ...
    const broc_reg_params *params)
{
    ...
    cuda_reg::QuadratureFilters qf = convert_filters(filters);
    cuda_reg::VolumeDims idims = { in_dims.W, in_dims.H, in_dims.D };
    cuda_reg::VoxelSize  ivox  = { in_vox.x, in_vox.y, in_vox.z };
    cuda_reg::VolumeDims rdims = { ref_dims.W, ref_dims.H, ref_dims.D };
    cuda_reg::VoxelSize  rvox  = { ref_vox.x, ref_vox.y, ref_vox.z };
    ...
    cuda_reg::T1MNIResult mr = cuda_reg::registerT1MNI(
        input, idims, ivox,
        ref, rdims, rvox,
...
static void cuda_destroy(broc_backend *self)
{
    free(self);
}
...
extern "C" broc_backend *broc_cuda_create_backend(void)
{
    ...
    b->register_volumes = cuda_register_volumes;
    b->destroy          = cuda_destroy;
    b->name             = "CUDA";
    b->priv             = NULL;
